...
2. 确认 `kooai-market-data` MCP 已完成认证。认证信息仅用于 MCP 访问，不读取、不打印、不写入命令参数、报告 JSON、页面或日志。
...
8. 启动本地独立选品页面服务，确认返回本地回环地址。端口被未知程序占用时停止，不结束未知进程。
...
| 检查项 | 必须满足 |
|---|---|
| 数据边界 | 只使用已认证 KooAI MCP 返回的数据，不绕过 MCP 查询其他数据源 |
